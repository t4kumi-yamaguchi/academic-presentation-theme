 ...
 *
 * 図版・イラストの取扱い:
 *  - スライド内に説明用のイラストやピクトグラムを挿入する場合は、
 *    https://github.com/t4kumi-yamaguchi/make-research-diagram のイラスト表現を参照する。
 *  - 参照するのはイラストの視覚表現のみとする。白背景または透過背景、黒〜中間グレー、
 *    均一な細い線画、影・光沢・3D・写真風表現なしを基本とする。
 *  - 同リポジトリの段階型、二層型、左から右の時系列、工程表、カード列、矢印構成は
 *    自動的に踏襲しない。図の配置と関係性は、各スライドの内容とユーザーの指示に基づく。
 *  - イラスト画像には日本語、数値、群名、時点、矢印ラベルを描き込まない。
 *    必要な文字、枠、矢印は PowerPoint 上で別要素として配置する。
 *  - グラフ、表、統計図はこのイラスト規則の対象外とし、本テーマの配色・書式を使う。
 *
 * 改行・禁則の取扱い(重要):
